...
export const SERVER_ENV_REL_PATH = ".grok/server-env.json";

const SERVER_ENV_ALLOW = new Set(["GEMINI_API_KEY"]);

/** Server-only secrets (never VITE_ / never sent to the browser). */
export function readServerEnv(root) {
  try {
    const parsed = JSON.parse(readFileSync(join(root, SERVER_ENV_REL_PATH), "utf8"));
    if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) return {};
    const env = {};
    for (const [key, value] of Object.entries(parsed)) {
      if (!SERVER_ENV_ALLOW.has(key)) continue;
